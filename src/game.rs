...
impl<R: TermRead + Read, W: Write> Game<R, W> {
    ...
    pub fn init(&mut self) {
        write!(self.stdout, "{}{}{}", cursor::Hide, clear::All, cursor::Goto(1, 1)).unwrap();
    }
    ...
    pub fn run(&mut self) {
        ...
        writeln!(self.stdout, "{}{}{}", clear::All, cursor::Goto(left, top), self.grid).unwrap();
        loop {
            let evt = self.events.next().unwrap().unwrap();
            ...
            if let Event::Key(key) = evt {
                ...
                if self.grid.is_solved() {
                    self.grid.freeze();
                    message = "You finished the puzzle! Press n to start a new one or q to quit".to_string();
                }
            }
            writeln!(self.stdout, "{}{}{}", clear::All, cursor::Goto(left, top), self.grid).unwrap();
